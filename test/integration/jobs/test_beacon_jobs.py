...
from test.factories import OpportunityFactory, VendorFactory, CategoryFactory, UserFactory
...
class TestBeaconJobs(BaseTestCase):
    ...
    @patch('beacon.jobs.job_base.EmailJobBase.run_job', side_effect=[JobStatus(status='skipped'), JobStatus(status='success')])
    @patch('beacon.jobs.beacon_nightly.AppStatus')
